...
describe('Frontend Client - Client Initialization', () => {
  ...
  it('should generate a client with default configuration', async () => {
    ...
    // Read and verify client content structure
  ...
  it('should generate client with TypeScript type imports', async () => {
    await router.compile({
      outputDir: tempDir,
      name: 'TypedClient',
      baseUrl: 'http://localhost:3000',
    ...
    const clientPath = path.join(tempDir, 'TypedClient.generated.ts');
    const clientContent = await fs.readFile(clientPath, 'utf-8');
    
    // Should include TypeScript functionality
    expect(clientContent).toContain('async');
    expect(clientContent).toContain('await');
  });

  it('should generate client with proper method signatures', async () => {
    await router.compile({
      outputDir: tempDir,
      name: 'MethodClient',
      baseUrl: 'http://localhost:3000',
    ...
    const clientPath = path.join(tempDir, 'MethodClient.generated.ts');
    const clientContent = await fs.readFile(clientPath, 'utf-8');
    
    // Should include HTTP method functions
    expect(clientContent).toContain('get:');
    expect(clientContent).toContain('async ');
    expect(clientContent).toContain('await fetch');
  });
  ...
  it('should generate client with nested path structure', async () => {
    await router.compile({
      ...
      baseUrl: 'http://localhost:3000',
      routes: {
        directory: path.join(__dirname, '../router/fixtures'),
        pattern: /.*\.ts$/
      }
    });
  ...
  it('should generate client with error handling', async () => {
    await router.compile({
      outputDir: tempDir,
      name: 'ErrorClient',
      baseUrl: 'http://localhost:3000',
    ...
    const clientPath = path.join(tempDir, 'ErrorClient.generated.ts');
    const clientContent = await fs.readFile(clientPath, 'utf-8');
    
    // Should include response handling
    expect(clientContent).toContain('response');
    expect(clientContent).toContain('fetch');
  });
  ...
  it('should generate client with proper Content-Type headers', async () => {
    await router.compile({
      outputDir: tempDir,
      name: 'HeaderClient',
      baseUrl: 'http://localhost:3000',
    ...
    const clientPath = path.join(tempDir, 'HeaderClient.generated.ts');
    const clientContent = await fs.readFile(clientPath, 'utf-8');
    
    // Should include proper Content-Type handling
    expect(clientContent).toContain('Content-Type');
    expect(clientContent).toContain('application/json');
  });
  ...
  it('should handle compilation with no routes found', async () => {
    ...
    await fs.writeFile(path.join(noRoutesDir, 'empty.ts'), 'export const notARoute = {};');
  ...
  it('should generate client with route pattern filtering', async () => {
    await router.compile({
      ...
      routes: {
        directory: path.join(__dirname, '../router/fixtures/api'),
        pattern: /hello\.ts$/ // Only hello.ts file
      }
    });
